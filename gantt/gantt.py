...
class Task(object):
        ...
        if self.cache_start_date != self.start:
            __LOG__.warning(
                '** starting date for task "{0}" is changed from {1} to {2}'.format(
                    self.fullname, self.start, self.cache_start_date
                )
            )
        return self.cache_start_date
    ...
    def end_date(self):
        ...
        if self.duration is None or self.start is None and self.stop is not None:
            ...
            if real_end <= self.start_date():
                ...
                real_duration = 0
                duration = (
                    (self.start_date() + self.duration) - self.start_date()
                ).days
                while duration > 1 or (
                    current_day.weekday() in _not_worked_days()
                    or current_day in VACATIONS
                ...
                self.cache_end_date = self.start_date() + datetime.timedelta(
                    days=real_duration
                )
                __LOG__.warning(
                    '** task "{0}" will not be finished on time : end_date is changed from {1} to {2}'.format(
                        self.fullname, self.stop, self.cache_end_date
                    )
                )
                return self.cache_end_date
    ...
    def svg(
        ...
        if (
            ...
            and self.start_date() != self.start
            and show_modified_dates
        ):
        ...
        if not title_align_on_left:
            tx = x + 2
            # Try to estimate if the title will hang over the right edge
            width_scale = 0.15
            if (len(self.fullname) * 15 * width_scale + tx) * mm > (
                _time_diff_d(end, start)
            ) * cm:
                tx = x - int((len(self.fullname) * 15 * width_scale)) + d

                __LOG__.warn(
                    f"Title {self.fullname} would overhang the right edge of the image, moving it to the left"
                )
        else:
            tx = 5
...
class Project(object):
    ...
    def _svg_calendar(
        self, maxx, maxy, start_date, today=None, scale=DRAW_WITH_DAILY_SCALE
    ):
        """
        ...
        for x in range(maxx):
            vlines.add(
                svgwrite.shapes.Line(
                    start=(x * cm, 2 * cm * YSCALE),
                    end=(x * cm, (maxy + 2) * cm * YSCALE),
                )
            )
            if scale == DRAW_WITH_DAILY_SCALE:
                ...
                # __LOG__.critical('DRAW_WITH_QUATERLY_SCALE not implemented yet')
                # sys.exit(1)
                jour = start_date + dateutil.relativedelta.relativedelta(
                    months=+(3 * x)
                )
        ...
        vlines.add(
            svgwrite.shapes.Line(
                start=(maxx * cm, 2 * cm * YSCALE),
                end=(maxx * cm, (maxy + 2) * cm * YSCALE),
            )
        )
        ...
        dwg.add(
            svgwrite.shapes.Line(
                ...
                end=((maxx) * cm, (2) * cm * YSCALE),
                stroke="black",
            )
        )
        dwg.add(
            svgwrite.shapes.Line(
                start=((0) * cm, (maxy + 2) * cm * YSCALE),
                end=((maxx) * cm, (maxy + 2) * cm * YSCALE),
                stroke="black",
            )
        )
        ...
        for y in range(2, maxy + 3):
            hlines.add(
                svgwrite.shapes.Line(
                    start=(0 * cm, y * cm * YSCALE), end=(maxx * cm, y * cm * YSCALE)
                )
            )
    ...
    def make_svg_for_tasks(
        ...
        psvg, pheight = self.svg(
            ...
            y_compact=y_compact,
            show_modified_dates=show_modified_dates,
        )
        if psvg is not None:
            ldwg.add(psvg)
        ...
        if custom_colors is not None:
            for color in custom_colors:
                ...
                ), "Custom colors should be valid svgwrite patterns or gradients"
                dwg.defs.add(color)
    ...
    def make_svg_for_resources(
        ...
        for r in resources:
            ...
            ress.add(
                svgwrite.text.Text(
                    "{0}".format(r.fullname),
                    insert=(3 * mm, (nline * 10 + 7) * mm * YSCALE),
                    fill=_font_attributes()["fill"],
                    stroke=_font_attributes()["stroke"],
                    stroke_width=_font_attributes()["stroke_width"],
                    font_family=_font_attributes()["font_family"],
            ...
            while cday <= end_date:
                ...
                if (
                    ...
                    vac.add(
                        svgwrite.shapes.Rect(
                            insert=(
                                ((cday - start_date).days * 10 + 1) * mm,
                                ((conflict_display_line) * 10 + 1) * mm * YSCALE,
                            ),
                ...
                if (
                    ...
                    conflicts.add(
                        svgwrite.shapes.Rect(
                            insert=(
                                ((cday - start_date).days * 10 + 1 + 4) * mm,
                                ((conflict_display_line) * 10 + 1) * mm * YSCALE,
                            ),
                            size=(4 * mm, 8 * mm * YSCALE),
                            fill="#AA0000",
